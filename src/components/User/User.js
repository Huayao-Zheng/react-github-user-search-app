import React from 'react';
import './User.css';

const User = ({ userProfile }) => {
  if (!userProfile) {
    return <></>;
  }

  const {
    name,
    login,
    created_at,
    bio,
    public_repos,
    followers,
    following,
    avatar_url,
    location,
    blog,
    twitter_username,
  } = userProfile;

  return (
    <div className="user">
      <div className="user__title-container">
        <div className="user__avatar-container">
          <img className="user__avatar" src={avatar_url} alt="avatar" />
        </div>
        <div className="user__names-container">
          <div className="user__name">{name}</div>
          <div className="user__login">{login}</div>
          <div className="user_joined-day">{created_at}</div>
        </div>
      </div>

      <div className="user__bio">{bio ? bio : 'Oh! Such empty...'}</div>

      <div className="user__repos-followers-following-contianer">
        <div className="user__repos">
          Repos
          <br />
          <strong>{public_repos}</strong>
        </div>
        <div className="user__followers">
          Followers
          <br />
          <strong>{followers}</strong>
        </div>
        <div className="user__following">
          Following
          <br />
          <strong>{following}</strong>
        </div>
      </div>

      <div className="user__links-container">
        <div className="user__location">
          <svg
            className="user__location-icon"
            height="20"
            width="14"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              d="M12.797 3.425C11.584 1.33 9.427.05 7.03.002a7.483 7.483 0 00-.308 0C4.325.05 2.17 1.33.955 3.425a6.963 6.963 0 00-.09 6.88l4.959 9.077.007.012c.218.38.609.606 1.045.606.437 0 .828-.226 1.046-.606l.007-.012 4.96-9.077a6.963 6.963 0 00-.092-6.88zm-5.92 5.638c-1.552 0-2.813-1.262-2.813-2.813s1.261-2.812 2.812-2.812S9.69 4.699 9.69 6.25 8.427 9.063 6.876 9.063z"
              fill="#4b6a9b"
            />
          </svg>
          <span className="user__location-text">{location}</span>
        </div>
        <div className="user__blog">
          {blog.length > 0 ? (
            <>
              <img className="user__blog-icon" alt="blog" />
              <a
                className="user__blog__link"
                href={blog}
                target="_blank"
                rel="noreferrer"
              >
                {blog}
              </a>
            </>
          ) : (
            <>
              <img className="user-links-not-available" alt="blog" />
              'Not Available'
            </>
          )}
        </div>
        <div className="user__twitter">
          {!!twitter_username ? (
            <a
              className="user__twitter__link"
              href={`https://twitter.com/${twitter_username}`}
              target="_blank"
              rel="noreferrer"
            >
              {twitter_username}
            </a>
          ) : (
            'Not Available'
          )}
        </div>
        <div className="user__company"></div>
      </div>
    </div>
  );
};

export default User;
