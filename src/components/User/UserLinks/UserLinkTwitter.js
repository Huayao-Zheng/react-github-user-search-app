...
const UserLinkTwitter = ({ twitter_username }) => {
  const isTwitterNameAvalible = !!twitter_username;

  return (
    <div
      className={`${
        isTwitterNameAvalible
          ? 'user__twitter'
          : 'user__twitter link-not-avalible'
      }`}
    >
      ...
      <a
        ...
        rel="noreferrer"
      >
        {isTwitterNameAvalible ? `@${twitter_username}` : 'Not Avalible'}
      </a>
    </div>
  );
};
